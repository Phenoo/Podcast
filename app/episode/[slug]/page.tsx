...
export async function generateStaticParams() {
  ...
  return slugRoutes.map(async (slug) => ({
    ...
    metadata: await generateBlogPostMetadata({ slug }), 
...
const BlogPost = async ({ params: { slug }, metadata }: Props) => {
  ...
  if (!post) return null;
  //const metadata = await generateBlogPostMetadata({ slug });

   // Replace with your actual function to generate metadata
...
export async function generateStaticParams() {
  ...
  return slugRoutes.map((episode) => ({
    episode,
  }));
}

const EpisodePage = async ({ params: { episode } }: Props) => {
  const query = groq`*[_type=="episode" && slug.current == $episode][0]  {
    ...,
    categories[]->,
    sponsors[]->,
    relatedEpisodes[]->
  ...
  const clientFetch = cache(client.fetch.bind(client));
  const post = await clientFetch(query, { episode });

  if (!post) return null;
  
  // Call the generateMetadata function to get the metadata
  const metadata = await generateMetadata({ params: { episode } });

  return (
    <>
      {/* Use the returned metadata in NextSeo */}
      <NextSeo
        title={post.title}
